...
class RequestCriteria implements Criteria
{
     ...
     * @return mixed
     */
    public function apply2($model, Repository $repository)
    {

        $fieldsSearchable   = $model->getSearchableFields();
        $sortableFields     = $model->getSortableFields();
        $search             = $this->request->get(config('database.criteria.params.search', 'search'), null);
        $searchFields       = $this->request->get(
        ...
        if (isset($sortBy) && !empty($sortBy) && in_array($sortBy, $sortableFields)) {
